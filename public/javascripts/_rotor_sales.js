...
(function($){
	...
	var TourPersonView = Backbone.View.extend({
		...
		manager: function(evt){
			var htmlContents = 'Manager:';
			var parentcategory = g_categories[this.model.get('category')].assignedby;
			htmlContents += '<select class=\'managerselectbox\'>';
			for(var i = 0; i < data_persons.length; i++) {
				if(data_persons[i].category_status == parentcategory 
					&& data_persons[i].portal_status == ''){
					htmlContents += '<option value=\''+data_persons[i].id_person+'\'>'+data_persons[i].firstname+'</option>';
				}
			}
			htmlContents += '</select>';
			htmlContents += '<button class=\'managercancelbutton btn-mini\'>Cancel</button>'
			htmlContents += '<button class=\'managerassignbutton btn-mini\'>Assign</button>';
			$(this.el).children('.assigndiv').html(htmlContents);
		},
		managercancel: function(){
			$(this.el).children('.assigndiv').html('');
		},
		managerassign: function(){
			var selectobj = $(this.el).children('.assigndiv').children('select');
			var managerid = selectobj.val();
			var idx = getPersonIndex(this.model.get('id'));
			...
			/*var htmlContents = '';
			htmlContents += $(this.el).children('select').val();
			$(this.el).children('.assignedcontentdiv').html(htmlContents);
			$(this.el).children('.assigndiv').html('');*/
		},
		unrender: function(){
			$(this.el).remove();
	...
	var TourCategoryListView = Backbone.View.extend({
		...
		render: function(){
			...
			for(var id in g_portals) {
				bSkip = false;
				for(var idc in g_categories){
					if(g_categories[idc].assignedby=='' && g_categories[idc].tourportal == id) {
						bSkip = true;
						break;
					}
				}
				if( bSkip == false && g_portals[id].istour == 1){
					var cat = new ModelCategory();
					cat.set({
						name: id,
			...
			this.c_collection.each(this.appendCategory);
			for(i = 0; i < data_persons.length; i++) {
				if(g_categories[data_persons[i].category_status].assignedby=='') {
					continue;
				}
				if(typeof g_portals[data_persons[i].portal_status] == 'undefined')
				{
