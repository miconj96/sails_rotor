...
(function($){
	...
	var PersonView = Backbone.View.extend({
		...
		render: function(){
			...
			htmlContents += '<button class=\'unavailbutton btn-mini unavailable\'>U</button>';
			linkedPortals = g_categories[this.model.get('category')].portallinks;
			if(linkedPortals) {
				for( var i in linkedPortals) {
					if(typeof g_portals[linkedPortals[i]] != 'undefined' 
					&& g_portals[linkedPortals[i]].isbtavailonrotor == 1) {
						if(g_portals[linkedPortals[i]].scheduledtimeneed == 1) {
							htmlContents += '<button id="btn_'+linkedPortals[i]+'" class="scheduledtimebutton btn-mini '+g_portals[linkedPortals[i]].btclass+'">'+g_portals[linkedPortals[i]].btname+'</button>';
						} else {
							htmlContents += '<button id="btn_'+linkedPortals[i]+'" class="portalmovebutton btn-mini '+g_portals[linkedPortals[i]].btclass+'">'+g_portals[linkedPortals[i]].btname+'</button>';
						}
					}
				}
			}
	...
	var CategoryListView = Backbone.View.extend({
		...
		render: function(){
			...
			this.c_collection.each(function(cat){
				$('#ol_'+cat.get('name')).sortable({
				...
				$('#ol_'+cat.get('name')).disableSelection();
			});
			for(i = 0; i < data_persons.length; i++) {
	...
	var TourPersonView = Backbone.View.extend({
		...
		render: function(){
			...
			if(typeof g_categories[personinfo.category_status] != 'undefined' 
				&& g_categories[personinfo.category_status].assignedby != ''){
				htmlContents += '<div class="btn-group dropdown" style="margin-left: 2px; float:right;">';
				htmlContents +=		'<button class="managerbutton btn btn-mini dropdown-toggle openmanager" data-toggle="dropdown">M</button>';
				htmlContents +=		'<ul class="dropdown-menu">';
				htmlContents +=		'</ul>';
				htmlContents += '</div>';
			}
			htmlContents +=		'<div class="btn-group dropdown" style="float:right;">';
			htmlContents +=			'<button class="removeportalbutton btn btn-mini dropdown-toggle movein" data-toggle="dropdown">IN</button>';
			htmlContents +=		'</div>';
			htmlContents += "</div>";
